...
int xpos, ypos, xgoal, ygoal;      /// for å animere teksten ...
boolean img_check, img_check_init; /// For å sjekke om bildet er tilgjengelig ...
...
void setup() {
  ...
  manus = splice(manus, "Trykk på SPACE for å bla deg videre.", 0);
...
void lastInnTekst(int s, int t) {
  ...
  fill(0);
  textLeading(42);
  text(msg, xgoal+xpos, ygoal+ypos, 400, 300);
  fill(230, 230, 220);
  text(msg, xgoal+xpos+1, ygoal+ypos+1, 400, 300);

  textSize(18);
  text("Side: "+s, 570, 25); /// sidetallet oppe i høyre hjørne
  text("Timer: "+t, 570, 45);  /// timer ... usikker på om det er noe bruk for den, men dog ...

  if ( abs(xgoal - xpos) > 0) { xpos *= 0.9;}
  if ( abs(ygoal - ypos) > 0) { ypos *= 0.9;}
  println( "abs(xgoal - xpos) = " + abs(xgoal - xpos) );
  println( "xpos = " + xpos );
}
